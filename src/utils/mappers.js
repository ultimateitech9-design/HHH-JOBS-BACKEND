const { isSalaryDisclosed } = require('./jobSalary');
...
const mapJobFromRow = (row) => {
  const salaryDisclosed = isSalaryDisclosed(row);
  ...
  return {
    ...
    salaryDisclosed,
    minPrice: salaryDisclosed ? row.min_price : null,
    maxPrice: salaryDisclosed ? row.max_price : null,
    salaryType: salaryDisclosed ? row.salary_type : null,
    ...
    pincode: row.pincode || '',
    applicationMode: row.application_mode || 'internal',
    externalApplyUrl: row.external_apply_url || '',
    applyUrl: row.external_apply_url || '',
    isPaid: row.is_paid,
