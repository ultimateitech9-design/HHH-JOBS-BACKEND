...
const parseAtsAiPayload = (text = '') => {
  ...
  try {
    ...
    return {
      ...
      interviewReadiness: truncate(parsed?.interviewReadiness || parsed?.readiness, 80),
      firstFix: truncate(parsed?.firstFix || parsed?.nextBestAction, 180),
      rewriteBullets: uniqueStrings(parsed?.rewriteBullets || parsed?.bulletRewrites, 4),
      keywordPlacement: uniqueStrings(parsed?.keywordPlacement || parsed?.keywordPlacementPlan, 6),
...
const buildAtsAiPrompts = ({ jobRow, resumeText, baseAnalysis }) => {
  ...
  const userPrompt = [
    ...
    '  "interviewReadiness": "Ready | Needs edits | Not ready",',
    '  "firstFix": "one highest-impact edit the candidate should make first",',
    '  "rewriteBullets": ["ATS-safe rewritten resume bullet grounded in existing resume evidence"],',
    '  "keywordPlacement": ["missing keyword -> where it should be added, only if genuinely supported"],',
    '  "calibrationDelta": 0,',
    ...
    '- Rewrite bullets must not invent metrics, companies, tools, or outcomes. If evidence is weak, write an honest strengthening direction.',
    '- Keyword placement must say where to add the keyword: headline, summary, skills, project, or experience bullet.',
    '',
    ...
    `Project evidence score: ${Number(baseAnalysis?.projectEvidenceScore || 0)}`,
    `Domain fit score: ${Number(baseAnalysis?.domainFitScore || 0)}`,
    ...
    `Project evidence lines: ${(baseAnalysis?.projectEvidenceLines || []).join(' | ') || 'None'}`,
    `Matched domains: ${(baseAnalysis?.matchedDomains || []).join(', ') || 'None'}`,
    `Missing domains: ${(baseAnalysis?.missingDomains || []).join(', ') || 'None'}`,
...
const enhanceAtsAnalysisWithAi = async ({ jobRow, resumeText = '', baseAnalysis }) => {
  ...
  try {
    ...
    return {
      ...
      aiInterviewReadiness: aiPayload.interviewReadiness,
      aiFirstFix: aiPayload.firstFix,
      aiRewriteBullets: aiPayload.rewriteBullets,
      aiKeywordPlacement: aiPayload.keywordPlacement,
