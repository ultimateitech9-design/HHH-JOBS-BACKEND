...
const resolveResumeInput = async ({ req, source }) => {
  ...
  if (source === 'profile_resume' && req.user.role === ROLES.HR) {
    return { error: 'HR should use custom resume upload/text/url source.' };
  }
...
const createAtsResult = ({ jobRow, resumeText = '', resumeUrl = '' }) => {
  const analysis = runAtsAnalysis({ jobRow, resumeText: resumeText || '' });

  if (!resumeText && resumeUrl) {
    analysis.warnings = [...analysis.warnings, 'Resume text could not be extracted from URL. Score may be lower.'];
...
router.post('/check-preview', asyncHandler(async (req, res) => {
  ...
  const previewJob = {
    job_title: targetText ? 'Custom Target Role' : 'General ATS Benchmark',
    company_name: 'HHH Job',
    description: targetText || 'General professional role requiring communication, structured resume sections, and relevant skill keywords.',
  ...
  const analysis = createAtsResult({
    jobRow: previewJob,
...
router.post('/check/:jobId', asyncHandler(async (req, res) => {
  ...
  if (checkError || !checkRow) {
    saved = false;
    persistenceWarning = checkError?.message
      || 'ATS result generated but history could not be saved.';
  } else {
    const atsItems = [
