...
router.use(requireAuth, requireActiveUser, requireRole(ROLES.HR, ROLES.ADMIN));
...
router.get('/jobs/:id/applicants', requireApprovedHr, asyncHandler(async (req, res) => {
  ...
  const job = await assertJobOwnership(jobId, req.user, res);
  if (!job) return;
  const canViewContacts = req.user.role === ROLES.ADMIN || Boolean(job.contact_details_visible);
