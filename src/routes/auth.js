...
const OAUTH_ALLOWED_ROLES = new Set([ROLES.STUDENT, ROLES.HR]);
...
const getRoleRedirectPath = (role) => (role === ROLES.ADMIN ? '/admin' : role === ROLES.HR ? '/hr' : '/student');
...
const buildOAuthClientRedirectUrl = ({ token, user, redirectTo, error }) => {
  ...
  if (user) {
    redirectUrl.searchParams.set('user', Buffer.from(JSON.stringify(user), 'utf-8').toString('base64url'));
  }
...
const createLocalRoleProfile = (role, userId, reqBody = {}) => {
  authStore.createRoleProfile(role, userId, {
...
router.get('/oauth/:provider/start', asyncHandler(async (req, res) => {
  ...
  const oauthConfig = provider.getClientConfig();
  if (!oauthConfig.clientId || !oauthConfig.clientSecret || !oauthConfig.redirectUri) {
    res.status(500).send({
      status: false,
      message: `OAuth is not configured for provider "${providerKey}".`
    });
    return;
  }
...
router.get('/oauth/:provider/callback', asyncHandler(async (req, res) => {
  ...
  const oauthConfig = provider.getClientConfig();
  if (!oauthConfig.clientId || !oauthConfig.clientSecret || !oauthConfig.redirectUri) {
    redirectOAuthFailure(res, `OAuth is not configured for provider "${providerKey}".`);
    return;
  }
...
router.post('/signup', asyncHandler(async (req, res) => {
  ...
  if (![ROLES.STUDENT, ROLES.HR].includes(requestedRole)) {
    res.status(400).send({ status: false, message: 'Invalid role. Allowed roles: student, hr' });
    return;
  }
  ...
  // Log OTP to console for testing (replace with email/SMS service in production)
  console.log(`[OTP] User ${email} OTP: ${otpCode} (expires in ${OTP_EXPIRY_MINUTES} min)`);
...
router.post('/verify-otp', asyncHandler(async (req, res) => {
  ...
  res.send({
    ...
    token,
    user: mapPublicUser({ ...user, is_email_verified: true }),
    redirectTo: user.role === ROLES.ADMIN ? '/admin' : user.role === ROLES.HR ? '/hr' : '/student'
  });
}));
...
router.post('/forgot-password', asyncHandler(async (req, res) => {
  ...
  console.log(`[OTP] Password reset for ${email} OTP: ${otpCode} (expires in ${OTP_EXPIRY_MINUTES} min)`);
...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  res.send({
    status: true,
    token,
    user: mapPublicUser({ ...userRow, last_login_at: loginTimestamp }),
...
router.get('/me', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!supabase) {
    profile = authStore.getProfileByRole(req.user.role, req.user.id);
