...
const store = {
  ...
  support: {
    stats: () => {
      const totalTickets = state.support.tickets.length;
      const openTickets = state.support.tickets.filter((item) => item.status === 'open').length;
      const pendingTickets = state.support.tickets.filter((item) => item.status === 'pending').length;
      const resolvedTickets = state.support.tickets.filter((item) => item.status === 'resolved').length;
      const escalatedTickets = state.support.tickets.filter((item) => item.status === 'escalated').length;
      ...
      return {
        ...
        resolvedTickets,
        escalatedTickets,
        liveChats: state.support.chats.length,
        complaints: state.support.complaints.length,
        feedbackItems: state.support.feedback.length,
        avgResolutionHours: state.support.stats.avgResolutionHours
      };
    },
  ...
  dataentry: {
    dashboard: () => {
      const dashboard = buildDataEntryDashboard();
      return {
        totals: {
          ...dashboard.totals,
          companiesAdded: dashboard.companyWorkflow.companyDetailsAdded || dashboard.companyWorkflow.companiesAdded || 0,
          totalEntries: state.dataentry.entries.length
        },
        candidateWorkflow: {
          ...dashboard.candidateWorkflow,
          profileCreated: dashboard.candidateWorkflow.profileCreated || dashboard.candidateWorkflow.profilesCreated || 0,
          profilesCreated: dashboard.candidateWorkflow.profilesCreated || dashboard.candidateWorkflow.profileCreated || 0,
          resumeUploaded: dashboard.candidateWorkflow.resumeUploaded || dashboard.candidateWorkflow.resumesUploaded || 0,
          resumesUploaded: dashboard.candidateWorkflow.resumesUploaded || dashboard.candidateWorkflow.resumeUploaded || 0
        },
        companyWorkflow: {
          ...dashboard.companyWorkflow,
          companyDetailsAdded: dashboard.companyWorkflow.companyDetailsAdded || dashboard.companyWorkflow.companiesAdded || 0,
          companiesAdded: dashboard.companyWorkflow.companiesAdded || dashboard.companyWorkflow.companyDetailsAdded || 0
        },
        pipeline: {
          ...dashboard.pipeline
        },
        quality: {
          ...dashboard.quality,
          pendingReview: dashboard.quality.pendingReview || dashboard.quality.pending || 0
        },
        activityFeed: clone(dashboard.activityFeed)
      };
    },
    entries: (filters = {}) => {
    ...
    records: () => {
      const entries = state.dataentry.entries;
      const jobEntries = entries.filter((item) => item.type === 'job');
        ...
        new Map(
          entries
            ...
            .map((item) => [
              item.companyName,
              {
                id: `CMP-${String(item.companyName).replace(/\s+/g, '-').toUpperCase()}`,
                companyName: item.companyName,
                location: item.location,
                totalEntries: entries.filter((entry) => entry.companyName === item.companyName).length,
      ...
      return {
        summary: {
          ...
          totalNotifications: state.dataentry.notifications.length
        },
        jobs: clone(jobEntries),
        candidates: clone(candidates),
        companies: clone(companies),
        notifications: clone(state.dataentry.notifications),
        queue: {
          drafts: clone(entries.filter((item) => item.status === 'draft')),
          pending: clone(entries.filter((item) => item.status === 'pending')),
          approved: clone(entries.filter((item) => item.status === 'approved')),
          rejected: clone(entries.filter((item) => item.status === 'rejected'))
        }
      };
    }
  },
